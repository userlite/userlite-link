    ...
    this.subscriptions.add(
        atom.workspace.observeTextEditors(editor => {
            editor.buffer.onWillSave(event => {
                ...
                this.editorSaved(event.path, editor);
            });
            editor.onDidDestroy(event => {
                var fullPath = editor.getPath();
                if (!fullPath) return;

                var basePath = fullPath.substr(config.getItem('linkedProjectRoot').length);
...
Traffic.prototype.receive = function(message, cb) {
    ...
    log('traffic', ['Message received:', message[message.type] || message.type]);
...
Traffic.prototype.connect = function() {
    log('traffic', 'Pulling updates');
    this.pullUpdates();
    this.tabs.addEditors();
};
...
Traffic.prototype.editorSaved = function(fullPath, editor, force) {
    if (!editor) return;
    ...
    if (!force && !comment && editor.buffer.cachedDiskContents === editor.buffer.getText()) return;

    if (!fullPath) return log('warning', ['Unable to check file - no path.']);

    var bufferBasePath = fullPath.substr(config.getItem('linkedProjectRoot').length); // Plus one to remove the trailing slash
        ...
        var sendBuffer = ut.bufferize(
            {
                ...
                data: {
                    ...
                    diffs: JSON.stringify(diffs.getHTMLDiffs(editor.buffer.cachedDiskContents || '', editor.buffer.getText()))
                }
            },
...
Traffic.prototype.sendFile = function(fullPath) {
    ...
    if (fullPath.indexOf(config.getItem('linkedProjectRoot')) === 0) {
        ...
        var now = new Date().getTime() + '_' + Math.random();
        state.set({
            type: 'upload',
            message: 'Uploading ' + bufferBasePath,
            id: now
...
Traffic.prototype.deletePaths = function(routes) {
    log('traffic', ['Deleting Routes: ', routes]);
    var now = new Date().getTime();
    state.set({
        type: 'delete',
        message: 'Deleting ' + JSON.stringify(routes, null, 4),
        id: now
    });
    ...
    this.socket.send(
        ...
        function(res) {
            ...
            routes.forEach(route => {
                fsExtra.remove(path.join(root, route));
            });
        }
    );
};

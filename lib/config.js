...
function missingOptions() {
    atom.notifications.addWarning('Missing required configuration options.', {
...
if (atom.inDevMode()) {
    if (!items.connectToDev || !items.connectToDevPort) missingOptions();
    ...
    if (!items.connectTo || !items.connectToPort) missingOptions();
    // SSL on live

...
ManagerActions.prototype.showComment = function(id) {
    this.comment.id = id;
    var version = this.versions.find(ver => ver.id === id);

    this.comment.saved = '';
    if (version) this.comment.saved = version.comment;

    // Set the diffs for the view
...
ManagerActions.prototype.cancelComment = function() {
    this.comment.id = false;
    this.comment.saved = '';
    this.manager.render();
};

ManagerActions.prototype.saveComment = function(options) {
    this.socket.send(
        {
            type: 'task',
            task: 'set-version-comment',
            data: options
        },
        res => {
            if (res.success) {

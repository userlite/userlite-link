...
State.prototype.devProfileHTML = function(devId) {
    ...
    span.className = 'dev-profile-icon inline-block' + (profile.appman ? ' appman' : '');
...
State.prototype.showDevs = function(profiles) {
    ...
    this.devProfiles.map(prof => {
        ...
        elem.addEventListener('click', event => {
            var res = atom.commands.dispatch(event.target, 'userlite-link:send-current-to-dev');
        });
...
State.prototype.devConfirm = function(devId) {
    var devElement = document.querySelector('.ullink-state-devs .dev-profile-icon[data-devid="' + devId + '"');

...
export default {
    ...
    config: {
        ...
        linkedProjectRoot: {
            type: 'string',
            description:
                'Provide the full path to the folder to link with the dev environment.',
            order: 3,
            default: ''
        },
        connectTo: {
    ...
    activate(initialState) {
        ...
        this.subscriptions.add(
            atom.commands.add('atom-workspace', {
                ...
                'userlite-link:delete-selected': () => {
                    if (!this.setup || !this.socket.connected)
                        return console.warn('not setup');

                    var deleteDirs = Array.from(
                    ...
                    deleteDirs = deleteDirs.reduce((prev, route) => {
                        if (
                            route.indexOf(
                    ...
                    atom.confirm({
                        message: 'Delete these paths?',
                        detailedMessage: deleteDirs.join(',\n'),
                        buttons: {
                            Continue: this.traffic.deletePaths.bind(
                                this.traffic,
                                deleteDirs
                            ),
                            Cancel: () => {}
